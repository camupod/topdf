...
void initializeOptions (Handle<Object> source, topdf_options* destination) {
    ...
    if (source->Has(String::New("compression"))) {

        // get the provided setting
        Local<Boolean> compression = source->Get(String::New("compression"))->ToBoolean();

        // set the baton value
        destination->compression = compression->Value() ? 0 : 1;

    } else {

        // default is to use compression
        destination->compression = 1;
